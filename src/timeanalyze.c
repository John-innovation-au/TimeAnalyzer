...
#define HOTHIT_TIME_INTERVAL    1000
...
char* m_pszMainFileName = NULL;
char* m_pszSubFileName = NULL;
char* m_pszExtFileName = NULL;
...
{
    int nRet = 0;
    if( pszMainFileName && pszSubFileName && pszExtFileName )
    {
        if( m_pszMainFileName )
        {
            free( m_pszMainFileName );
            m_pszMainFileName = NULL;
        }
        m_pszMainFileName = malloc( strlen(pszMainFileName) + 1 );
        memcpy( m_pszMainFileName, pszMainFileName, strlen(pszMainFileName) + 1 );
        
        if( m_pszSubFileName )
        {
            free( m_pszSubFileName );
            m_pszSubFileName = NULL;
        }
        m_pszSubFileName = malloc( strlen(pszSubFileName) + 1 );
        memcpy( m_pszSubFileName, pszSubFileName, strlen(pszSubFileName) + 1 );
        
        if( m_pszExtFileName )
        {
            free( m_pszExtFileName );
            m_pszExtFileName = NULL;
        }
        m_pszExtFileName = malloc( strlen(pszExtFileName) + 1 );
        memcpy( m_pszExtFileName, pszExtFileName, strlen(pszExtFileName) + 1 );
...
int TimeAnalyzeSaveAllData()
{
    ...
    //NSString *strHomePath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    //NSString* strMainFileName = [NSString stringWithFormat:@"%@/main.txt", strHomePath];
    //NSString* strSubFileName = [NSString stringWithFormat:@"%@/sub.txt", strHomePath];
    //NSString* strExtFileName = [NSString stringWithFormat:@"%@/ext.txt", strHomePath];
    //
    ...
    //TimeAnalyzeItemInit();
    //
    //SetupSaveFileNames( pszMainFileName, pszSubFileName, pszExtFileName );

    if( m_pszMainFileName && m_pszSubFileName && m_pszExtFileName )
    {
        nRet = TimeAnalyzeSaveData( m_pszMainFileName, m_pszSubFileName, m_pszExtFileName );
    }
#else
#endif
    return nRet;
            ...
            for( unsigned int i = 0; i<MAX_ITEM_COUNT; ++i )
            {
                if( m_pSubItems[i] )
                {
                    ...
                    fprintf( pf2, "\n%4d%8d%8d%8llu%8d%8llu%8d%8llu%8d%8d%8d%8d%s",
                            ...
                            m_pSubItems[i]->nHotLostCount,
                            m_pSubItems[i]->nHotLostTime,
                            m_pSubItems[i]->nLastDiff,
                            m_pSubItems[i]->nZeroValueCount,
                            m_pSubItems[i]->nUpValueSum,
                            m_pSubItems[i]->nDownValueSum,
